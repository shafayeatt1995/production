	...
	<div>
		...
		<nav class="navbar navbar-expand-lg main-navbar" :class="sidebar ? 'active':''">
			...
			<ul class="navbar-nav navbar-right">
				...
				<li class="dropdown">
					<a href="#" class="nav-link dropdown-toggle nav-link-lg nav-link-user" @click.prevent="showSection('menu')">
						<img alt="image" :src="`${url}${user.avatar}`" class="rounded-circle mr-1">
						<div class="d-sm-none d-lg-inline-block">Hi, {{user.name}}</div>
					</a>
					<div class="dropdown-menu dropdown-menu-right" :class="show === 'menu' ? 'show':''">
						<a href="features-settings.html" class="dropdown-item has-icon">
							<i>
								<icon :icon="['fas', 'cog']"></icon>
							</i>
							Settings
						</a>
		...
		<transition name="fade" mode="out-in">
			<div class="sidebar-overlay mobile-only" v-if="!sidebar" @click="toggleSidebar">
...
<script>
	export default {
		...
		methods: {
			...
			showSection(section) {
				this.show = this.show !== section ? section : "";
			},
