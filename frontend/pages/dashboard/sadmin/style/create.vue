...
<script>
	export default {
		...
		head() {
			return {
				title: `Create Style - ${this.appName}`,
			};
		},
		...
		methods: {
			...
			submit() {
				if (this.click) {
					this.click = false;
					this.errors = {};
					this.$axios.post("create-style", this.form).then(
						(response) => {
							$nuxt.$emit("success", response.data);
							this.$router.push({ name: "dashboard-sadmin-style" });
							this.click = true;
						},
						(error) => {
