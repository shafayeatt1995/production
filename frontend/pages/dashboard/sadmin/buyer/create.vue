						...
						<div class="image-form">
							<div class="image-frame">
								...
								<label for="logo" class="image-frame-content" v-else>
									...
									<span>Select and upload buyer logo</span>
								</label>
							</div>
...
<script>
	export default {
		...
		head() {
			return {
				title: `Create Buyer - ${this.appName}`,
			};
		},
		...
		methods: {
			...
			submit() {
				if (this.click) {
					...
					this.$axios.post("create-buyer", formData, config).then(
						(response) => {
							$nuxt.$emit("success", response.data);
							this.$router.push({ name: "dashboard-sadmin-buyer" });
							this.click = true;
						},
						(error) => {
